...
namespace vietlabs.h2
{
    internal class h2_Active : h2_Icon
    {
        ...
        internal void Toggle(GameObject go) // Click
        {
            ...
            if (h2_Selection.PartOfMuti(go))
            {
                ...
                var undoName = (v ? "Deactive " : "is_active ") + arr.Length + " GameObjects";
                SetActiveArray(v, arr, undoName);
            }
            else
            {
                var undoName = (v ? "Deactive " : "is_active ") + go.name;
        ...
        internal void ToggleParent(GameObject go)
        {
            ...
            while (p != null)
            {
                Undo.RecordObject(p.gameObject, "Toggle is_active Parent");
        ...
        {
            Undo.IncrementCurrentGroup();
            SetActiveArray(!go.activeSelf, h2_Unity.GetSiblings(go).ToArray(), "Toggle is_active Siblings");
        }
        ...
        internal void ToggleChildren(GameObject go)
        {
            ...
            Undo.IncrementCurrentGroup();
            Undo.RecordObject(go, "Toggle is_active Children");
            go.SetActive(v);

            foreach (Transform t in go.transform)
            {
                Undo.RecordObject(t.gameObject, "Toggle is_active Children");
        ...
        internal void ActivePrevSibling(GameObject go)
        {
            ...
            Undo.IncrementCurrentGroup();
            SetActiveArray(false, siblings.ToArray(), "is_active Prev Sibling", prev);
            SetActive(prev, true, "is_active Prev Sibling", h2_ParentAction.Active);
        ...
        internal void ActiveNextSibling(GameObject go)
        {
            ...
            Undo.IncrementCurrentGroup();
            SetActiveArray(false, siblings.ToArray(), "is_active Prev Sibling", next);
            SetActive(next, true, "is_active Prev Sibling", h2_ParentAction.Active);
    ...
    internal class h2_ActiveIconSetting : h2_IconSetting
    {
        ...
        static readonly string[] ACTIVE_STATES = {"is_active", "Inactive", "Self"};

        static readonly string[] ACTIVE_SHORTCUTS =
        {
            "is_active", CMD_TOGGLE, "_V",
            "is_active Siblings", CMD_TOGGLE_SIBLING, string.Empty,
            "is_active Parent", CMD_TOGGLE_PARENT, string.Empty,
            "is_active Children", CMD_TOGGLE_CHILDREN, string.Empty,
            "is_active Prev Sibling", CMD_ACTIVE_PREV_SIBLING, "_<",
            "is_active Next Sibling", CMD_ACTIVE_NEXT_SIBLING, "_>"
        };
